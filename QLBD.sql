...
--1.CHO BT THÔNG TIN CẦU THỦ 
SELECT MACT,HOTEN,NGAYSINH,DIACHI,SO
FROM CAUTHU
--2 CẦU THỦ SỐ 7 CHƠI TIỀN VỆ
SELECT MACT,HOTEN,VITRI,NGAYSINH,DIACHI,SO, TENQG, TENCLB
FROM CAUTHU, QUOCGIA, CAULACBO
WHERE CAUTHU.MAQG = QUOCGIA.MAQG AND CAUTHU.MACLB = CAULACBO.MACLB AND SO = 7 AND VITRI = N'Tiền Vệ'
--3 TÊN, NGÀY SINH, DC ,SDT CÙA HLV
SELECT TENHLV,CONVERT(VARCHAR, NGAYSINH, 103) AS NGAYSINH ,DIACHI,DIENTHOAI
FROM HUANLUYENVIEN
--Câu 4:TT CẦU THỦ THUỘC CLB: BBD VÀ QUỐC GIA VN
Select HOTEN, VITRI, NGAYSINH, DIACHI from CAUTHU CT JOIN QUOCGIA Q ON CT.MAQG=Q.MAQG
JOIN CAULACBO CL ON CT.MACLB=CL.MACLB
where MACLB='BBD' and MAQG='VN'
--5 
SELECT MACT , HOTEN , NGAYSINH , DIACHI , VITRI 
FROM CAUTHU JOIN CAULACBO 
ON CAUTHU.MACLB = CAULACBO.MACLB 
JOIN QUOCGIA 
ON CAUTHU.MAQG = QUOCGIA.MAQG 
WHERE CAULACBO.TENCLB= 'SHB ĐÀ NẴNG ' 
AND
      QUOCGIA.TENQG = 'BRA-XIN' 
--6
SELECT *
FROM dbo.CAUTHU CT INNER JOIN dbo.CAULACBO CLB ON CLB.MACLB = CT.MACLB
				   INNER JOIN dbo.SANVD SAN ON SAN.MASAN = CLB.MASAN
WHERE SAN.TENSAN LIKE N'Long An'
--7
select
MATRAN,CONVERT(VARCHAR,NGAYTD,103) AS NGAYTD,SAN.TENSAN, CLB1.TENCLB AS TENCLB1,CLB2.TENCLB AS TENCLB2 , KETQUA
...
WHERE td.VONG = 2 and year(td.NGAYTD) =2019
--8
...
--11 THỐNG KÊ SL CẦU THỦ CỦA MỖI CLB
SELECT CLB.MACLB,TENCLB, COUNT(CT.MACT) AS [SỐ LƯỢNG CẦU THỦ]
FROM CAULACBO CLB INNER JOIN CAUTHU CT ON  CLB.MACLB=CT.MACLB
GROUP BY TENCLB,CLB.MACLB
--12 TK SL CẦU THỦ NC NGOÀI MỖI CLB
SELECT CT.MACLB, COUNT(CT.MACT) AS [SL CAU THU NGOẠI]
FROM CAUTHU CT JOIN QUOCGIA QG ON CT.MAQG=QG.MAQG
WHERE QG.TENQG <> 'VIỆT NAM'
GROUP BY CT.MACLB
--13 MACLB, TÊN CLB, TÊN SVĐ,DC,SL CẦU THỦ NC NGOÀI VS NHỮNG CLB CÓ NHIỀU HƠN 2 CT NN
SELECT CL.MACLB,CL.TENCLB,S.TENSAN,S.DIACHI,COUNT(CT.MACT) AS [SL CẦU THỦ NGOẠI]
FROM CAUTHU CT JOIN CAULACBO CL ON CT.MACLB = CL.MACLB
JOIN SANVD S ON CL.MASAN=S.MASAN
GROUP BY CL.MACLB,CL.TENCLB,S.TENSAN,S.DIACHI
HAVING COUNT(CT.MACT) >2
--14 TÊN TỈNH, SL CẦU THỦ TĐ CỦA CLB DO TỈNH ĐÓ QL
SELECT T.TENTINH, COUNT(CT.MACT) AS [SL TIỀN ĐẠO]
FROM CAUTHU CT JOIN CAULACBO CL ON CT.MACLB=CL.MACLB
JOIN TINH T ON CL.MATINH=T.MATINH
WHERE T.MATINH LIKE CL.MATINH
GROUP BY T.TENTINH
...
--15 TÊN CLB, TÊN TỈNH MÀ CLB NẰM TOP 1 VÒNG 3, 2019
SELECT CL.TENCLB,T.TENTINH
FROM CAULACBO CL JOIN TINH T ON CL.MATINH=T.MATINH
JOIN BANGXH XH ON CL.MACLB=XH.MACLB
WHERE XH.HANG = 1 AND XH.VONG = 3 AND XH.NAM = 2019
--16
SELECT TENHLV
FROM HUANLUYENVIEN
...
INNER JOIN SANVD SAN ON TD.MASAN=SAN.MASAN
...
--Câu 23. Cho biết mã HLV, họ tên, ngày sinh, địa chỉ của những HLV Việt Nam có tuổi nằm trong khoảng 35 - 40:
...
SELECT LEFT(HIEUSO, PATINDEX('%-%', HIEUSO) - 1)
from BANGXH
--Câu 26:
...
/*28.Cho biết danh sách các trận đấu (NGAYTD, TENSAN, TENCLB1, TENCLB2, KETQUA) của câu lạc bộ CLB có thứ hạng thấp nhất 
trong bảng xếp hạng vòng 3 năm 2019.*/
...
JOIN CAULACBO CLB2 ON TD.MACLB2 = CLB2.MACLB
WHERE TD.MACLB1 IN (SELECT MACLB FROM BANGXH WHERE VONG = 3 AND NAM = 2019 AND DIEM 
<= ALL (SELECT DIEM FROM BANGXH)
 OR TD.MACLB2 IN (SELECT MACLB FROM BANGXH WHERE VONG = 3 AND NAM = 2019 AND DIEM 
<= ALL (SELECT DIEM FROM BANGXH WHERE VONG = 3 AND NAM = 2019)))
--câu 29:
SELECT MACLB, TENCLB
FROM CAULACBO CLB
WHERE (SELECT COUNT(MATRAN) FROM TRANDAU 
WHERE CLB.MACLB IN (TRANDAU.MACLB1, TRANDAU.MACLB2) AND NAM = 2019) = (SELECT COUNT(MACLB) -1 FROM CAULACBO)
--Câu 30: 
SELECT MACLB, TENCLB
FROM CAULACBO CLB
...
--Cau 33
create trigger trg_KTTuoiCT on CAUTHU for insert as
begin
	declare @Tuoi int
	select @Tuoi = DATEDIFF(YEAR,CAUTHU.NGAYSINH, GETDATE()) from CAUTHU 
	if(@Tuoi < 18)
	begin
		print('Khong du tuoi')
		rollback tran
